...
native_osx_apache_php5_module="LoadModule php5_module libexec\/apache2\/libphp5.so"
native_osx_apache_php7_module="LoadModule php7_module libexec\/apache2\/libphp7.so"
...
# Formula for latest version of PHP doesn't inclue any @<version>
brew_php_formula_to_link="php@$given_php_version"
if [ "$given_php_version" == "$latest_brew_php_version" ]; then
    brew_php_formula_to_link="php"
fi


if [ $(echo "$given_php_version" | sed 's/\.//') -ge 70 ]; then
...
apache_php_mod_path="$php_opt_path\/$given_php_version\/$apache_php_lib_path"
...
if [[ (" ${valet_support_php_version_array[*]} " != *"$brew_php_formula_to_link"*) && ($valet_restart -eq 1) ]]; then
    echo "Sorry, but $brew_php_formula_to_link is not support via valet" 2>&1
    exit 1
fi

# Check that the requested version is supported
if [[ " ${php_array[*]} " != *"$brew_php_formula_to_link"* ]]; then
    echo "Unknown version of PHP. PHP Switcher can only handle arguments of:" ${brew_array[@]} 2>&1
    exit 1
fi

# Check that the requested version is installed
if [[ " ${php_installed_array[*]} " != *"$brew_php_formula_to_link"* ]]; then
    echo "Sorry, but $brew_php_formula_to_link is not installed via brew. Install by running: brew install $brew_php_formula_to_link" 2>&1
    exit 1
fi
...
# Switch Shell
echo "Switching to $brew_php_formula_to_link"
...
brew link --force "$brew_php_formula_to_link"
...
if [[ $apache_change -eq 1 ]]; then
    ...
    for j in ${php_installed_array[@]}; do
        ...
        if [ $(echo "$j" | sed 's/^php@//') == "$latest_brew_php_version" ]; then
            j="php"
        fi
        ...
        # Else the string for the php module is not in the apache config then add it
            ...
            if grep -q "$native_osx_apache_php7_module" "$apache_conf_path"; then
                sudo sed -i.bak "/$native_osx_apache_php7_module/a\\
$comment_apache_module_string\\
" $apache_conf_path
            else
                sudo sed -i.bak "/$native_osx_apache_php5_module/a\\
$comment_apache_module_string\\
" $apache_conf_path
            fi
        fi
